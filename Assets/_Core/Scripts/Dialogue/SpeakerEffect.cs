...
namespace MoonlitMixes.Dialogue.Effect
{
    public class SpeakerEffect : MonoBehaviour
    {
        ...
        private IEnumerator TrembleEffect()
        {
            ...
            // D�but du tremblement
            for (int i = 0; i < 20; i++)
            {
                // Applique une variation al�atoire � la position de l'image
                Vector3 offset = new Vector3(Random.Range(-intensityX, intensityX), Random.Range(-intensityY, intensityY), 0);
                transform.localPosition = originalPosition + offset;

                // Applique un tremblement � la position du texte
                if (_linkedText)
                {
                ...
                // Variation l�g�re de la rotation de l'image pour un effet de tremblement
